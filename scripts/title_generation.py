...
hdlr = logging.FileHandler('log/train.20170531.log')
...
word2idx, idx2word,X_train, X_test, Y_train, Y_test = pickle.load(open(DataFile))
...
num_filters = 16
sample_file = "log/train.20170601.samples"
...
with tf.Session() as sess: 
    ...
    for i in range(epocs):
        ...
        while (j < len(X_train)):
            ...
            j = j + batch_size
            _,loss_,decoder_prediction_ = sess.run([train_op,loss,decoder_prediction],
                feed_dict={
            ...
            if j % (batch_size * 30) == 0:
                ...
                test_encode_input = rpadd(X_test[k],maxlend)
                test_decode_output = rpadd(Y_test[k],maxlenh,prefix=beg)
        ...
        if i %  save_epoc_step == 0:
                saver.save(sess,"model3/TitleGeneration",global_step = i)
